...
router.get("/me", authenticateToken, async (req, res) => {
  try {
    ...
    user.lastEntryDate =
      lastEntry.rows.length > 0 ? lastEntry.rows[0].created_at : null; // Add last entry date
    res.json(user);
  } catch (err) {
...
router.put("/me/settings", authenticateToken, async (req, res) => {
  ...
  const snakeCaseFields = allowedFields.map((field) =>
...
router.delete("/me", authenticateToken, async (req, res) => {
  const password_hash = await pool
    .query("SELECT password_hash FROM users WHERE id = $1", [req.user.id])
