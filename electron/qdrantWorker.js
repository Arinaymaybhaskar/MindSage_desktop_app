...
async function upsertJournalToQdrant(journal, embedding) {
    try {
        ...
        db.prepare("UPDATE journal_entries SET qdrant_id = ? WHERE id = ?").run(qdrantId, journal.id);
...
async function upsertGoalToQdrant(goal, embedding) {
    try {
        ...
        db.prepare("UPDATE goals SET qdrant_id = ? WHERE id = ?").run(qdrantId, goal.id);
...
async function upsertProgressLogToQdrant(progressLog, goalTitle, embedding) {
    try {
        ...
        await client.upsert('mind_entries', { points: [point] });
        parentPort?.postMessage(`Successfully upserted progress log ${progressLog.id} to Qdrant`);
        db.prepare("UPDATE progress_logs SET qdrant_id = ? WHERE id = ?").run(qdrantId, progressLog.id);
        return true;
    } catch (error) {
...
async function updateQdrantPayload(entry, sourceType = 'journal') {
    try {
        if (!entry.qdrant_id) {
            ...
            parentPort?.postMessage(`Created new Qdrant point for ${sourceType} ${entry.id}`);
            return true;
        }
        ...
        switch (sourceType) {
            case 'journal':
                ...
                textToEmbed = entry.title ? `${entry.title} ${entry.content}` : entry.content;
                payload = {
            ...
            case 'goal':
                pointId = entry.qdrant_id;
                textToEmbed = entry.title ? `${entry.title} ${entry.description || ''}` : entry.description;
                payload = {
                    user_id: entry.user_id,
                    source_type: 'goal',
                    source_id: entry.id,
        ...
        parentPort?.postMessage(`Successfully updated payload for ${sourceType} ${entry.id} in Qdrant`);
        return true;
    } catch (error) {
        console.error(`[QDRANT-WORKER] Error updating Qdrant payload for ${sourceType} ${entry.id}:`, error);
        throw error;
    }
}
...
async function processGoal(goal) {
    try {
        ...
        const textToEmbed = goal.title ? `${goal.title} ${goal.description || ''}` : goal.description;
        parentPort?.postMessage(`Generating embedding for goal ${goal.id}...`);

        const embedding = await generateEmbedding(textToEmbed);
        parentPort?.postMessage(`Embedding generated for goal ${goal.id}, length: ${embedding.length}`);
        ...
        try {
            ...
            parentPort?.postMessage(`Note: Could not update synced_to_qdrant for goal ${goal.id} - column may not exist`);
        }

        parentPort?.postMessage(`Goal ${goal.id} processed and synced successfully`);

    } catch (error) {
        parentPort?.postMessage(`Error processing goal ID: ${goal.id} - ${error.message}`);
...
async function processProgressLog(progressLog) {
    try {
        ...
        const goal = db.prepare(
            `SELECT title, unit, user_id FROM goals WHERE id = ?`
        ...
        parentPort?.postMessage(`Generating embedding for progress log ${progressLog.id}...`);

        const embedding = await generateEmbedding(textToEmbed);
        parentPort?.postMessage(`Embedding generated for progress log ${progressLog.id}, length: ${embedding.length}`);
        ...
        try {
            ...
            // Column might not exist yet, that's okay
            parentPort?.postMessage(`Note: Could not update synced_to_qdrant for progress log ${progressLog.id} - column may not exist`);
        }

        parentPort?.postMessage(`Progress log ${progressLog.id} processed and synced successfully`);

    } catch (error) {
        parentPort?.postMessage(`Error processing progress log ID: ${progressLog.id} - ${error.message}`);
...
eventBus.on("progress_log:created", async ({ entry }) => {
    parentPort?.postMessage(`Received progress_log:created event for progress log ${entry.id}`);
    try {
        const updateStmt = db.prepare(
...
parentPort?.on('message', async (message) => {
    ...
    if (message.type === 'journal:sync-requested') {
        ...
        if (goal) {
            parentPort?.postMessage(`Found goal ${goalId} in database, processing...`);
        ...
        parentPort?.postMessage(`Received manual sync request for progress log ${progressLogId}`);
        console.log(`Worker: Received manual sync request for progress log ${progressLogId}`);

        const progressLog = db.prepare(
            `SELECT * FROM progress_logs WHERE id = ?`
        ).get(progressLogId);

        if (progressLog) {
            parentPort?.postMessage(`Found progress log ${progressLogId} in database, processing...`);
            console.log(`Worker: Found progress log ${progressLogId} in database, processing...`);
            await processProgressLog(progressLog);
        } else {
            parentPort?.postMessage(`Progress log ${progressLogId} not found`);
            console.log(`Worker: Progress log ${progressLogId} not found in database`);
        }
        ...
        for (const goal of pendingGoals) {
            try {
                ...
                parentPort?.postMessage(`Error processing goal ${goal.id}: ${error.message}`);
                console.log(`Worker: Error processing goal ${goal.id}: ${error.message}`);
            }
        }
        ...
        const pendingProgressLogs = db.prepare(
            `SELECT * FROM progress_logs ORDER BY logged_at DESC`
        ).all();

        parentPort?.postMessage(`Found ${pendingProgressLogs.length} progress logs to sync`);
        console.log(`Worker: Found ${pendingProgressLogs.length} progress logs to sync`);

        for (const progressLog of pendingProgressLogs) {
            try {
                await processProgressLog(progressLog);
            } catch (error) {
                parentPort?.postMessage(`Error processing progress log ${progressLog.id}: ${error.message}`);
                console.log(`Worker: Error processing progress log ${progressLog.id}: ${error.message}`);
            }
        }
        ...
        const { entry } = message.data;
        parentPort?.postMessage(`Received goal:qdrant-update-needed message for goal ${entry.id}`);

        try {
            await updateQdrantPayload(entry, 'goal');
            parentPort?.postMessage(`Successfully updated Qdrant payload for goal ${entry.id}`);
        } catch (error) {
            parentPort?.postMessage(`Error updating Qdrant payload for goal ${entry.id}: ${error.message}`);
        }
    } else if (message.type === 'progress_log:qdrant-update-needed') {
        const { entry } = message.data;
        parentPort?.postMessage(`Received progress_log:qdrant-update-needed message for progress log ${entry.id}`);
        ...
        try {
            ...
            parentPort?.postMessage(`Successfully updated Qdrant payload for progress log ${entry.id}`);
        } catch (error) {
            parentPort?.postMessage(`Error updating Qdrant payload for progress log ${entry.id}: ${error.message}`);
        }
    }
});
...
parentPort?.postMessage("Qdrant worker started with event-based architecture for journals, goals, and progress logs.");
console.log("Qdrant worker started with event-based architecture for journals, goals, and progress logs.");
