...
export async function exportEverything(userId, destinationPath) {
    ...
    try {
        ...
        tempDir = path.join(fs.existsSync('/tmp') ? '/tmp' : os.tmpdir(), exportFolderName);
        ...
        const userQuery = db.prepare('SELECT id, username, email, full_name, timezone, profile_picture, created_at FROM users WHERE id = ?');
        const userSettingsQuery = db.prepare('SELECT * FROM user_settings WHERE user_id = ?');
        const journalEntriesQuery = db.prepare('SELECT * FROM journal_entries WHERE user_id = ? ORDER BY created_at DESC');
        ...
        const aiInterventionsQuery = db.prepare('SELECT * FROM ai_interventions WHERE user_id = ?');
        const aiNudgesQuery = db.prepare('SELECT * FROM ai_nudges WHERE user_id = ?');
        const userEmotionPatternsQuery = db.prepare('SELECT * FROM user_emotion_patterns WHERE user_id = ?');
        const categoriesQuery = db.prepare('SELECT * FROM categories WHERE user_id = ?');
        const goalsQuery = db.prepare('SELECT * FROM goals WHERE user_id = ?');
        const progressLogsQuery = db.prepare('SELECT * FROM progress_logs WHERE goal_id IN (SELECT id FROM goals WHERE user_id = ?)');
        const chatsQuery = db.prepare('SELECT * FROM chats WHERE user_id = ? ORDER BY created_at DESC');
        const messagesQuery = db.prepare('SELECT * FROM messages WHERE chat_id IN (SELECT id FROM chats WHERE user_id = ?)');
        const filesQuery = db.prepare('SELECT * FROM files WHERE chat_id IN (SELECT id FROM chats WHERE user_id = ?)');
        const messageSourcesQuery = db.prepare('SELECT * FROM message_sources WHERE message_id IN (SELECT id FROM messages WHERE chat_id IN (SELECT id FROM chats WHERE user_id = ?))');
        ...
        const copyAsset = (sourcePath, destDir, destFilename) => {
            if (sourcePath && fs.existsSync(sourcePath)) {
                ...
                } catch (error) { console.error(`Failed to copy asset: ${sourcePath}`, error); }
            } else if (sourcePath) {
                // Log a warning if the source file doesn't exist to make debugging easier.
                console.warn(`Asset source file not found, skipping copy: ${sourcePath}`);
            }
        };
        ...
        if (sourceData.user && sourceData.user.profile_picture) {
            ...
            flatData.user.profile_picture = path.join('images', baseFilename).replace(/\\/g, '/');
            structuredData.user.profile_picture = path.join('..', '..', '..', 'images', baseFilename).replace(/\\/g, '/');
        }
        ...
        sourceData.journal_entries.forEach((entry, index) => {
            if (entry.image_key) {
                ...
                copyAsset(sourcePath, rootImagesPath, baseFilename);
                flatData.journal_entries[index].image_key = path.join('images', baseFilename).replace(/\\/g, '/');
                structuredData.journal_entries[index].image_key = path.join('..', '..', '..', 'images', baseFilename).replace(/\\/g, '/');
            }
            if (entry.audio_key) {
                ...
                flatData.journal_entries[index].audio_key = path.join('audio', baseFilename).replace(/\\/g, '/');
                structuredData.journal_entries[index].audio_key = path.join('..', '..', '..', 'audio', baseFilename).replace(/\\/g, '/');
            }
        });
        ...
        sourceData.files.forEach((file, index) => {
            ...
            flatData.files[index].file_path = path.join(destFolder, baseFilename).replace(/\\/g, '/');
            structuredData.files[index].file_path = path.join('..', '..', '..', destFolder, baseFilename).replace(/\\/g, '/');
        });
        ...
        const metadata = { exported_at: new Date().toISOString(), app_version: '1.0.0', data_schema_version: '1.3.0-combined' };
