...
async function installOllama() {
    ...
    try {
        ...
        emitProgress("installing-ollama", { step: "running-installer", percent: 100 });
        await new Promise((resolve, reject) => {
        ...
        emitProgress("installing-ollama", { step: "manual-fallback", error: err.message });
...
async function pullModel(modelName) {
    ...
    try {
        ...
        emitProgress("pulling-model", { model: modelName, status: "success", percent: 100 });
        return { success: true };
    } catch (err) {
        emitProgress("pulling-model", { model: modelName, status: "error", error: err.message });
...
export function registerSetupIPC() {
    ...
    ipcMain.handle("setup:delete-model", (_e, modelName) => deleteModel(modelName));
    ...
    ipcMain.handle("setup:ensure-embedding", async () => {
        ...
        if (hasModel(models, EMBEDDING_MODEL)) return { success: true, alreadyPresent: true };
