...
try {
    ...
    console.log('Columns before migration:', beforeInfo.map(col => col.name));

    // Run the migration
    const userSettingsInfo = db.prepare(`PRAGMA table_info(user_settings)`).all();
    const hasCustomColors = userSettingsInfo.some(col => col.name === 'custom_colors');
    const hasSelectedTheme = userSettingsInfo.some(col => col.name === 'selected_theme');
    const hasUseCustomColors = userSettingsInfo.some(col => col.name === 'use_custom_colors');
    ...
    if (!hasSelectedTheme) {
        db.prepare(`ALTER TABLE user_settings ADD COLUMN selected_theme TEXT DEFAULT 'Default'`).run();
        console.log('Added selected_theme column');
    }
    if (!hasUseCustomColors) {
        db.prepare(`ALTER TABLE user_settings ADD COLUMN use_custom_colors INTEGER DEFAULT 0`).run();
    ...
    console.log('Columns after migration:', afterInfo.map(col => col.name));
    ...
    const result = db.prepare('SELECT * FROM user_settings WHERE user_id = 1').get();
