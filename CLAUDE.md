...
- **A hardcoded JWT secret still ships inside the packaged app** (`offlineAccessTokenSecret`, [electron/methods/auth.js:9](electron/methods/auth.js#L9)), and eight modules under `electron/methods/` call `jwt.decode` rather than `jwt.verify`. Both are tracked blockers, so don't extend the pattern.
