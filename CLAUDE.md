...
- **Renderer imports omit the file extension** (`import App from "./App"`). `allowImportingTsExtensions` is on, so both forms resolve, and 5 stray imports under `src/` do carry an explicit `.ts`/`.tsx`. The other 248 do not. This file used to document those 5 as the convention; it was describing the minority. Match the 248.
...
- **`viteStaticCopy` copies only what `qdrantWorker.js` needs at runtime**: the worker itself, `eventBus.js`, and `db/connection.js`. Everything else under `electron/` is bundled into `main.js`, so adding a file there needs no config change. The targets used to be `db/*`, `methods/*` and `services/*`, which shipped about 25 files as a second, dead copy of code already in the bundle. Only add a target if `qdrantWorker.js` gains an import: the worker is spawned from a file path rather than imported, so its dependencies have to exist as real files, and missing one breaks the packaged build while dev mode keeps working.
